  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let* ([v1 (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? v1)
               (eval-under-env (fun-body (closure-fun v1)) new_env)
               (error "MUPL closure not given")))]
...
(define (mlet*-helper lstlst env)
  (let ([new_env (if (null? lstlst)
                     null
                     (cons (car (car lstlst)) (eval-under-env (cdr (car lstlst)) env)))])
  (if (null? lstlst)
      null
      (cons new_env (mlet*-helper (cdr lstlst) (cons new_env env))))))

(define (mlet* lstlst e2) (eval-under-env e2 (mlet*-helper lstlst null)))
